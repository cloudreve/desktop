...
mod inner {
    ...
    pub fn send_general_text_toast(_title: &str, _message: &str) {
        tracing::debug!(target: "toast", "Toast notifications not supported on this platform");
    }

    pub fn send_token_expiry_toast(_drive_id: &str, _title: &str, _message: &str) {
        tracing::debug!(target: "toast", "Toast notifications not supported on this platform");
    }

    pub fn send_conflict_toast(_drive_id: &str, _path: &PathBuf, _inventory_id: i64) {
        tracing::debug!(target: "toast", "Toast notifications not supported on this platform");
    }
}
